...
export class AiService {
  ...
  async generateConversation(
  ...
  ): Promise<{ script: { speaker: string; text: string; translation?: string }[] }> {
    ...
    // Nếu includeVietnamese: tăng thêm 50% cho translation
    const baseTokens = minExchanges * 300 + 500;
    const estimatedTokens = includeVietnamese ? Math.ceil(baseTokens * 1.5) : baseTokens;
    ...
    // Phần instruction cho Vietnamese translation
    const vietnameseInstruction = includeVietnamese
      ? `
- MỖI EXCHANGE phải có thêm trường "translation": bản dịch tiếng Việt TỰ NHIÊN của "text"
- Dịch nghĩa chứ KHÔNG dịch word-by-word`
      : '';

    const vietnameseFormat = includeVietnamese
...
{
  "script": [
    { "speaker": "Person A", "text": "..."${vietnameseFormat} },
    { "speaker": "Person B", "text": "..."${vietnameseFormat} }
  ]
}
