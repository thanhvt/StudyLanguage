...
|----|---------|-------------|
| NAV-01 | Home hiển thị 4 mode cards: Practice, Shadowing, AI Conversation, Tongue Twister | P0 |
| NAV-02 | Home hiển thị Daily Goal card với progress ring + streak | P0 |
| NAV-03 | Tap "Xem Dashboard →" trên Daily Goal → navigate đến Progress Dashboard | P1 |
| NAV-04 | Tap ⚙️ (top-right) → mở TTS Settings Bottom Sheet (overlay, không rời Home) | P0 |
| NAV-05 | Home **KHÔNG** hiển thị topics/scenarios. Chọn topic xảy ra trong config screen của mỗi mode | P0 |
| NAV-06 | Hỗ trợ cả Dark Mode và Light Mode | P0 |
| NAV-07 | First-time user → hiện Onboarding Overlay (5 steps) | P2 |

#### Non-Functional Requirements

| ID | Yêu cầu | Chi tiết |
|----|---------|----------|
| NAV-NF01 | Thời gian load Home ≤ 300ms | Không fetch API khi mở Home |
| NAV-NF02 | Animation mode cards | Fade-in staggered khi first mount |
| NAV-NF03 | Haptic feedback | Light haptic khi tap mode card |
| NAV-NF04 | Accessibility | VoiceOver labels cho mọi interactive element |
...
| ID | Yêu cầu | Mức ưu tiên |
|----|---------|-------------|
| PRC-01 | Config screen: chọn topic bằng **Listening Topic Picker** (reuse component) | P0 |
| PRC-02 | Config screen: chọn level Beginner / Intermediate / Advanced | P0 |
| PRC-03 | Session screen: hiển thị câu mẫu (English) + IPA (toggle ẩn/hiện) | P0 |
| PRC-04 | Nút "Nghe mẫu" → phát audio câu bằng Azure TTS | P0 |
| PRC-05 | Hold mic → ghi âm (max 15s) + waveform animation | P0 |
| PRC-06 | Release mic → submit audio → AI scoring | P0 |
| PRC-07 | Swipe up khi đang ghi → hủy recording | P0 |
| PRC-08 | Feedback: overall score (0-100 + grade), word-by-word, phoneme heatmap | P0 |
| PRC-09 | Feedback: AI tips phát âm + AI Voice Clone (Before/After) | P1 |
| PRC-10 | Navigation: "Nói lại" (retry) và "Câu tiếp theo" (next) | P0 |
| PRC-11 | Confetti animation khi score ≥ 90 | P2 |
| PRC-12 | Preview recording trước khi submit (toggle setting) | P2 |
| PRC-13 | Countdown 3→2→1→GO! trước recording (toggle setting) | P2 |
| PRC-14 | Tap vào từ bất kỳ trong feedback → popup IPA + audio phát âm chuẩn | P1 |
| PRC-15 | Auto-save session vào history khi hoàn thành | P0 |
| PRC-16 | Share result dưới dạng image card → social media | P2 |
