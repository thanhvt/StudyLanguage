...
| ID | Yêu cầu | Mức ưu tiên | Trạng thái |
|----|---------|-------------|------------|
| SHD-01 | Config screen: chọn topic bằng **Listening Topic Picker** (reuse component) | P0 | ✅ Done |
| SHD-02 | Config screen: chọn speed (0.5x — 1.5x, default 1.0x) | P0 | ✅ Done |
| SHD-03 | Config screen: chọn delay (slider 0s — 2.0s, default 0.5s) | P1 | ✅ Done |
| SHD-04 | Config screen: chọn scoring mode (Post-recording / Realtime Beta) | P2 | ✅ Done |
| SHD-05 | Config screen: hiển thị headphone status + recommendation | P0 | ✅ Done |
| SHD-06 | Preview phase: AI phát câu mẫu + text highlight sync | P0 | ✅ Done |
| SHD-07 | Shadow phase: AI phát lại + user ghi âm đồng thời + dual waveform | P0 | ✅ Done |
| SHD-08 | Score phase: Rhythm / Intonation / Accuracy breakdown (0-100 each) | P0 | ✅ Done |
| SHD-09 | Score phase: waveform comparison overlay (AI vs User) | P1 | ✅ Done |
| SHD-10 | Action phase: Repeat / Next / Share | P0 | ✅ Done |
| SHD-11 | Auto-stop recording khi AI phát xong | P0 | ✅ Done |
| SHD-12 | Session Summary khi hết danh sách câu | P0 | ✅ Done |

### 1.2 Non-Functional Requirements

| ID | Yêu cầu | Chi tiết | Trạng thái |
|----|---------|----------|------------|
| SHD-NF01 | Audio latency ≤ 50ms | Đồng bộ playback + recording không trễ | ⚠️ Cần test device |
| SHD-NF02 | Dual waveform render ≤ 16ms/frame | 60fps cho cả 2 tracks | ✅ Done (200ms poll) |
| SHD-NF03 | AEC (Acoustic Echo Cancellation) | Tự động bật khi không có tai nghe | ✅ Done |
| SHD-NF04 | Haptic feedback | Medium khi bắt đầu shadow, Light khi phase kết thúc | ✅ Done |
| SHD-NF05 | Memory | 2 audio streams + waveform data ≤ 30MB RAM | ⚠️ Cần test device |
...
| TC-ID | Tên | Steps | Expected | Trạng thái |
|-------|-----|-------|----------|------------|
| SHD-TC01 | Chọn topic | Topic picker → chọn "Business" | Card highlight + ✓ | ✅ Impl |
| SHD-TC02 | Chọn custom scenario | Tab ✨ → chọn custom | Card highlight + ✓ | ✅ Impl |
| SHD-TC03 | Speed selection | Tap "0.75x" | Purple border, khác default | ✅ Impl |
| SHD-TC04 | Delay slider | Drag slider tới 1.5s | Label "1.5s" cập nhật | ✅ Impl |
| SHD-TC05 | Headphone detected | Cắm tai nghe | "✅ Tai nghe kết nối" green | ✅ Impl |
| SHD-TC06 | No headphone | Rút tai nghe | "⚠️ Nên dùng tai nghe" warning | ✅ Impl |
| SHD-TC07 | Start without topic | Tap CTA không chọn topic | Button disabled / toast | ✅ Impl |
| SHD-TC08 | Start thành công | Chọn topic + config → tap CTA | Loading → navigate Shadowing Screen | ✅ Impl |

### 8.2 Phase 1 — Preview

| TC-ID | Tên | Steps | Expected | Trạng thái |
|-------|-----|-------|----------|------------|
| SHD-TC10 | AI phát câu | Vào Phase 1 | AI TTS phát + text highlight sync | ✅ Impl |
| SHD-TC11 | Speed applied | Config 0.75x → Phase 1 | AI phát chậm hơn bình thường | ✅ Impl |
| SHD-TC12 | Shadow button | AI phát xong | "🎤 Shadow!" button hiện lên | ✅ Impl |
| SHD-TC13 | Progress | Câu 3/8 | "Câu 3/8" + progress bar 37.5% | ✅ Impl |

### 8.3 Phase 2 — Shadow (Active)

| TC-ID | Tên | Steps | Expected | Trạng thái |
|-------|-----|-------|----------|------------|
| SHD-TC20 | Dual waveform | Tap "Shadow!" | AI waveform (purple) + User waveform (green) render | ✅ Impl |
| SHD-TC21 | Recording auto-start | Phase 2 bắt đầu | Mic tự động bắt đầu ghi + haptic medium | ✅ Impl |
| SHD-TC22 | Text highlight sync | Đang shadowing | Từ đang phát → bold highlight | ✅ Impl |
| SHD-TC23 | Auto-stop | AI phát xong | Recording tự dừng → haptic light → Phase 3 | ✅ Impl |
| SHD-TC24 | AEC active | Không tai nghe + đang Shadow | AI audio giảm 30%, AEC bật | ✅ Impl |

### 8.4 Phase 3 — Score

| TC-ID | Tên | Steps | Expected | Trạng thái |
|-------|-----|-------|----------|------------|
| SHD-TC30 | Score loading | Phase 2 → 3 | "AI đang so sánh..." loading indicator | ✅ Impl |
| SHD-TC31 | Score display | Score loaded | 3 rings: Rhythm + Intonation + Accuracy | ✅ Impl |
| SHD-TC32 | Waveform overlay | Score loaded | AI (purple) vs User (green) overlay | ✅ Impl |
| SHD-TC33 | Segment play AI | Tap "Nghe AI" | Phát audio AI | ✅ Impl |
| SHD-TC34 | Segment play User | Tap "Nghe tôi" | Phát audio User | ✅ Impl |
| SHD-TC35 | Alternating play | Tap "Xen kẽ" | Phát AI → User → AI → User | ✅ Impl |
| SHD-TC36 | Tips display | Score loaded | AI tips hiển thị | ✅ Impl |

### 8.5 Phase 4 — Action

| TC-ID | Tên | Steps | Expected | Trạng thái |
|-------|-----|-------|----------|------------|
| SHD-TC40 | Repeat | Tap "🔄 Lặp lại" | Quay về Phase 1, cùng câu | ✅ Impl |
| SHD-TC41 | Next | Tap "→ Câu tiếp" | Phase 1, câu tiếp theo | ✅ Impl |
| SHD-TC42 | Share | Tap "📤 Share" | Capture score card → share sheet | ✅ Impl |
| SHD-TC43 | Last sentence | Câu cuối → tap Next | Navigate → Session Summary | ✅ Impl |
| SHD-TC44 | Session Summary | Hết câu | Tổng kết: avg scores + thời gian + tips | ✅ Impl |
...
| Case | Trigger | Xử lý | Rủi ro | Trạng thái |
|------|---------|-------|--------|------------|
| Không cắm tai nghe | User bắt đầu | Warning modal: "Nên dùng tai nghe 🎧" + cho phép tiếp | ⚠️ Medium | ✅ Done |
| Rút tai nghe giữa session | Jack/BT disconnect | Toast + auto-pause + switch AEC mode | 🔴 High | ✅ Done |
| AEC không hiệu quả | Loa ngoài + mic kém | Score thấp → tips "Thử dùng tai nghe" | ⚠️ Medium | ⚠️ API side |
| User nói quá nhỏ | Mic không bắt | Toast "Hãy nói to hơn nhé!" + low amplitude indicator | ⚠️ Medium | ✅ Done |
| Device nóng (thermal) | CPU cao do dual stream | Auto-pause + "Thiết bị cần nghỉ" (hiếm gặp) | ✅ Low | ❌ Thấp |

### 9.2 Recording & Playback

| Case | Trigger | Xử lý | Rủi ro | Trạng thái |
|------|---------|-------|--------|------------|
| AI audio fail load | TTS server lỗi | Toast + retry: "Không thể phát câu mẫu" | 🔴 High | ✅ try/catch |
| Recording overlap | User nói trước AI xong | Trim recording → align với AI | ⚠️ Medium | ⚠️ Phase 2 |
| Speed 1.5x quá nhanh | User không theo kịp | Score thấp → tips "Thử giảm tốc độ" | ✅ Low | ⚠️ API side |
| App minimize | User switch app | Auto-stop recording + save state → resume khi quay lại | ⚠️ Medium | ✅ Done |
| Incoming call | Cuộc gọi đến | Audio interrupt → auto-stop → resume sau | 🔴 High | ✅ Done (AppState) |

### 9.3 Network

| Case | Trigger | Xử lý | Rủi ro | Trạng thái |
|------|---------|-------|--------|------------|
| Mất mạng khi load sentences | Offline | Cache locally + "Mất kết nối" | 🔴 High | ⚠️ Phase 2 |
| Mất mạng khi scoring | Upload fail | Audio cache locally + retry khi có mạng | 🔴 High | ⚠️ Phase 2 |
| Score timeout (>15s) | Server chậm | Cancel + Toast + [Thử lại] | ⚠️ Medium | ✅ Done |
| Groq Whisper quota | Rate limit | Fallback model → toast nếu vẫn lỗi | 🔴 High | ⚠️ API side |

### 9.4 Potential Issues

| Issue | Mô tả | Mitigation | Trạng thái |
|-------|-------|------------|------------|
| **Audio sync drift** | AI playback và recording bị lệch timing | Timestamp-based alignment + NTP sync | ⚠️ Phase 2 |
| **Waveform memory leak** | 2 waveform listeners không cleanup | `removeRecordBackListener` trong useEffect cleanup | ✅ Fixed |
| **Speed affects STT** | 0.5x tạo ra audio dài → STT chậm hơn | Pre-trim silence + chunked upload | ⚠️ Phase 2 |
| **Dual audio session conflict** | playAndRecord + route switching | Set `AVAudioSession` category chính xác per headphone state | ⚠️ Cần test |
| **Bluetooth latency** | BT headphones có latency ~150ms | Bù delay tự động dựa trên BT codec detection | ⚠️ Phase 2 |
| **Battery drain** | Dual stream consume nhiều battery | Optimize polling interval cho waveform: 50ms thay 16ms | ✅ 200ms |
